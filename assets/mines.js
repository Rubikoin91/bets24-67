...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-1b06cc1d";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-1b06cc1d">'+'<span class="tile-face-1b06cc1d tile-front-1b06cc1d"></span>'+'<span class="tile-face-1b06cc1d tile-back-1b06cc1d"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-1b06cc1d")){return;}
var back=tile.querySelector(".tile-back-1b06cc1d");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-1b06cc1d");if(isMine){tile.classList.add("boom-1b06cc1d");}
if(dim){tile.classList.add("dimmed-1b06cc1d");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-1b06cc1d")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-1b06cc1d");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-1b06cc1d")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-1b06cc1d");void panelEl.offsetWidth;panelEl.classList.add("jolt-1b06cc1d");}
endRound();}
